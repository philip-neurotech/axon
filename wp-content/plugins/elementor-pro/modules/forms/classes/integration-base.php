...
abstract class Integration_Base extends Action_Base {
	...
	public static function global_api_control( $widget, $api_key = '', $label = '', $condition = [], $id = '' ) {
		if ( empty( $api_key ) ) {
			// translators: %1$s: Integration label, %2$s: Setting Page link
			$html = sprintf( __( 'Set your %1$s key in the <a href="%2$s" target="_blank">Integrations Settings</a>.', 'elementor-pro' ), $label, Settings::get_url() . '#tab-integrations' );
